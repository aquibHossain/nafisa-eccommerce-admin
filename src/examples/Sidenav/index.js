...
function Sidenav({ color, brand, brandName, routes, ...rest }) {
  ...
  const renderCollapse = (collapses) =>
    collapses?.filter((route) => route.key)?.map(({ name, collapse, route, href, key }) => {
      let returnValue;
  ...
  const renderRoutes = routes.filter((route) => route.key).map(
  ...
  return (
    <>
      {/* Open Button (Only visible when sidenav is minimized) */}
      {miniSidenav && (
        <Button
          variant="contained"
          color="primary"
          onClick={openSidenav}
          sx={{ position: "fixed", background: "white", top: 16, left: 16, zIndex: 1300 }}
        >
          <Icon>menu</Icon>
        </Button>
      )}
      <SidenavRoot {...rest} variant="permanent" ownerState={{ darkSidenav, miniSidenav, layout }}>
        <Box pt={3} pb={1} px={4} textAlign="center">
          ...
          <Box component={NavLink} to="/" display="flex" alignItems="center">
            {brand && <Box component="img" src={brand} alt="Amarlodge Logo" width="11rem" />}
          </Box>
        </Box>
        <Divider light={darkSidenav} />
        <List>{renderRoutes}</List>

        <Box pt={1} mt="auto" mx={2}>
          <SidenavFooter />
        </Box>
      </SidenavRoot>
    </>
