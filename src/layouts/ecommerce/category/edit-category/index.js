...
function EditCategory() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${config?.production_url}/category/${id}`, data, {
        headers: {
      ...
      setImagePreview(null); // Reset the image preview after successful submission
    } catch (err) {
      if (err.response) {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        const response = await axios.get(`${config?.production_url}/category/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        setFormData({
          ...
          image: fetchedCategory.image || null,
          featured: fetchedCategory.featured || false,
        });
        if (fetchedCategory.image) {
          setImagePreview(fetchedCategory.image); // If the category has an image, set the preview
        }
      } catch (error) {
