...
function CouponList() {
  ...
  useEffect(() => {
    const fetchcoupons = async () => {
      try {
        const response = await axios.get(`${config?.production_url}/coupon/admin/all`, {
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${config?.production_url}/coupon/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      setList((prevList) => prevList.filter((coupon) => coupon._id !== id));
          ...
          <Box display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            <Box lineHeight={1}>
              ...
              <Typography variant="button" fontWeight="regular" color="text">
                Manage your coupons efficiently with search and pagination.
              </Typography>
            </Box>
            <Stack spacing={2} direction="row">
              <Link to="/ecommerce/new-coupon">
              ...
              <TableBody>
                {list.map((coupon, index) => (
                  <TableRow key={coupon._id}>
                    ...
                    <TableCell align="center">{new Date(coupon.endDate).toLocaleDateString()}</TableCell>
                    <TableCell align="center">
                      <Link to={`/ecommerce/coupon/edit-coupon/${coupon._id}`}>
                        <RemoveRedEyeIcon />
                      </Link>
                    </TableCell>
                    <TableCell align="center">
