...
function SubCategory() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${config?.production_url}/sub-category`, data, {
        headers: {
      ...
      setFormData({ name: "", image: null, category: "", featured: false });
      setImagePreview(null); // Clear the preview after successful submission
    } catch (err) {
      if (err.response) {
        toast.error(err.response?.data?.message || "Failed to upload sub-category.");
      } else if (err.request) {
