...
function EditShipping() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${config.production_url}/shipping/${id}`, formData, {
        headers: {
      ...
      toast.error(
        err.response?.data?.message || "An unexpected error occurred while updating the shipping."
      );
  ...
  useEffect(() => {
    const fetchShipping = async () => {
      try {
        ...
        const response = await axios.get(`${config.production_url}/shipping/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });

        const fetchedShipping = response.data.data;
        setFormData({
          name: fetchedShipping.name || "",
          amount: fetchedShipping.amount || 0,
          description: fetchedShipping.description || "",
          active: fetchedShipping.active || false,
        });
      } catch (err) {
