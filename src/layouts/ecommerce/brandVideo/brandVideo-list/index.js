...
function BrandVideoList() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        const response = await axios.get(`${config?.production_url}/brand-video/admin/all`, {
          headers: {
  ...
  const handleAction = async (id) => {
    ...
    try {
      await axios.delete(`${config?.production_url}/brand-video/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      setList((prevList) => prevList.filter((brand) => brand._id !== id));

      toast.success("Brand Video Was Deleted Successfully");
    } catch (err) {
      console.error(err);
        ...
        <Card>
          <Box display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            ...
            <Stack spacing={1} direction="row">
              <Link to="/ecommerce/new-brand-video">
              ...
              <TableBody>
                {list.map((row, index) => (
                  ...
                  >
                    ...
                    <TableCell align="center">
                      <Link to={`/ecommerce/brand-video/edit-brand-video/${row?._id}`}>
                        <RemoveRedEyeIcon />
                      </Link>
                    </TableCell>
                    <TableCell align="center">
