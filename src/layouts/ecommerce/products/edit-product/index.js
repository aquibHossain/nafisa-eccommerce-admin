...
function NewProduct() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`${config?.production_url}/item/${productId}`, {
          headers: {
  ...
  const handleSubmit = async () => {
    try {
      ...
      Object.keys(formData).forEach((key) => {
        if (key === "productImage" && Array.isArray(formData[key])) {
          ...
          formData[key].forEach((imgObj) => {
            if (imgObj.file) {
              ...
              payload.append("productImage", imgObj)
            }
          });
        }
        else if (key === "image") {
        ...
        else if (key === "variants" && Array.isArray(formData[key])) {
          formData[key].forEach((variant, index) => {
            Object.keys(variant).forEach((variantKey) => {
              payload.append(`variants[${index}][${variantKey}]`, variant[variantKey]);
            });
          });
        }
          ...
          formData[key].forEach((item, index) => {
            Object.keys(item).forEach((field) => {
              if (field === "img" && item[field].file) {
                payload.append(`img${index}`, item[field].file);
              } else {

                payload.append(`inventory[${index}][${field}]`, item[field]);
              }
            });
          });
        }
      ...
      //   },
      // });
      const response = await axios.put(`${config?.production_url}/item/${productId}`, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "multipart/form-data",
        },
