...
function ProductsReviewList() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const { userReview: result, numberOfReviews: totalPages } = response.data.data?.reviews;
      setList(result);
  ...
  const handleAction = async (id, reviewId) => {
    try {
      const response = await axios.delete(`${config?.production_url}/item/product/review/admin/${id}/${reviewId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <DashboardLayout>
      ...
      <Modal open={isModalOpen} onClose={handleCloseModal}>
        ...
        >
          ...
          <ProductReviewEdit data={{ ...selectedOrder, productId: id }} handleCloseModal={handleCloseModal} fetchProducts={fetchProducts} />
