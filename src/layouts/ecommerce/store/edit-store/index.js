...
function EditStore() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${config.production_url}/store/${id}`, data, {
        headers: {
      ...
      toast.error(
        err.response?.data?.message || "An unexpected error occurred while updating the store."
      );
  ...
  useEffect(() => {
    const fetchBrand = async () => {
      try {
        ...
        const response = await axios.get(`${config.production_url}/store/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        setFormData({
          ...
          city: fetchedBrand.city || "",
          division: fetchedBrand.division || "",
          country: fetchedBrand.country || "",
          map: fetchedBrand.map || "",
        });
        setImagePreview(fetchedBrand.image || null); // Set the image preview URL for existing images
      } catch (err) {
